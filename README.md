...
| `config.realm`                              | kong                                       | false    | Realm used in the `WWW-Authenticate` response header. It must not contain control characters.                                                                                          |
...
| `config.redirect_uri`                       |                                            | false    | Internal callback path used after successful authentication. It must start with `/`. If omitted, the plugin derives it from the request path.                                        |
...
The redirect-related settings `config.redirect_uri`, `config.recovery_page_path`, `config.redirect_after_logout_uri`, and `config.post_logout_redirect_uri` are now restricted to internal paths. This keeps authentication and logout flows on the Kong-controlled surface instead of redirecting to arbitrary external destinations.
